...
for ( k in 1:nb_Iter){
  ...
  res_FPOP2<- FPOPDp(Data, penalty, type = 2)
...
######################################
png(filename = "Plot FPOP2 Candidates  data10^5 iter100.png",  width = 1500, height = 1000)

plot(means_candidates, main = "FPOP2:Candidates", xlab = "Time", ylab = "Number of candidates(mean) being considered", col = "royalblue3")
lines(means_candidates, col= "royalblue3")
abline(h = mean_value,lty = 2, col= "grey")
abline(v = chp, lty = 2, col = "grey")

dev.off()
write.table(means_candidates, "FPOP2 Candidates  data10^5 iter100.txt",  row.names = FALSE,col.names = FALSE  ) 
